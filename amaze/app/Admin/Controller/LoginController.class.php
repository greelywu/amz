...
class LoginController extends Controller {
    ...
    public function register()
    {
        ...
        if (IS_POST) {
            ...
            if ($id = $user->add($data)) {
				...
				session('uid', $id);          // 当前用户id
                session('nickname', $data['nickname']);   // 当前用户昵称
                session('username', $data['username']);   // 当前用户名
                session('lastdate', $data['lastdate']);   // 上一次登录时间
                session('lastip', $data['lastip']);       // 上一次登录ip
    ...
    public function logout()
    {
        // 清楚所有session
        session(null);
        redirect(U('Admin/Index/index/p/'.I('path.3')), 2, '正在退出登录...');
    }
